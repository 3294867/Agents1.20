...
const useHandleDropdownEnterKey = ({
    dropdownRef,
    isOpen,
    setIsOpen,
}: Props): void => {
    ...
    const handleKeyDown = useCallback(
        (event: KeyboardEvent) => {
            if (isOpen && dropdownRef.current && event.key == "Enter") {
                ...
                if (focusedElement) {
                    if (
                        focusedElement.id.startsWith("bookmark_thread_button")
                    ) {
                        const button =
                            focusedElement.closest<HTMLButtonElement>(
                                'button[id^="bookmark_thread_button"]',
                            );
                        if (button) {
                        ...
                        focusedElement.id.startsWith("delete_thread_button")
                    ) {
                        const button =
                            focusedElement.closest<HTMLButtonElement>(
                                'button[id^="delete_thread_button"]',
                            );
                        if (button) {
                            const workspaceName =
