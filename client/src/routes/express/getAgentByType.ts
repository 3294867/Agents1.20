...
const getAgentByType = async ({ agentType }: Props): Promise<Agent | null> => {
    ...
    try {
        const response = await fetch(
            `${import.meta.env.VITE_VITE_EXPRESS_URL}/api/get-agent-by-type`,
            {
                method: "POST",
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Failed to get agent (PostgresDB): ${response.status} | ${response.statusText} - ${errorText}`,
            );
        }
        ...
        if (Object.keys(data.data).length === 0) {
            throw new Error(
                `Incorrect format of the agent. Expected non-empty '{}'`,
            );
        }
