...
const deleteReqRes = async (req: Request, res: Response): Promise<void> => {
    ...
    const validationError = utils.validate.deleteReqRes({
        requestId,
        responseId,
    });
    if (validationError)
    ...
    `,
        ...
        if (getThreadId.rows.length === 0) {
            ...
            return utils.sendResponse({
                res,
                status: 404,
                message: "Failed to get thread id",
            });
        }
    ...
    `,
        ...
        if (deleteThreadRequest.rows.length === 0) {
            ...
            return utils.sendResponse({
                res,
                status: 503,
                message: "Failed to delete thread request",
            });
        }
    ...
    `,
        ...
        if (deleteThreadResponse.rows.length === 0) {
            ...
            return utils.sendResponse({
                res,
                status: 503,
                message: "Failed to delete thread response",
            });
        }
    ...
    `,
        ...
        if (getThreadBody.rows.length === 0) {
            ...
            return utils.sendResponse({
                res,
                status: 404,
                message: "Failed to get thread body",
            });
        }

        const filteredThreadBody: ReqResPG[] =
            getThreadBody.rows[0].body.filter(
                (q: ReqResPG) => q.request_id !== requestId,
            );
    ...
    `,
        ...
        if (updateThreadBody.rows.length === 0) {
            ...
            return utils.sendResponse({
                res,
                status: 503,
                message: "Failed to update thread body",
            });
        }
