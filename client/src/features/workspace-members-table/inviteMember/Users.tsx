...
const Users = () => {
    const { workspaceId, memberNames } =
        hooks.features.useWorkspaceMembersTableContext();
    ...
    const handleSearchValue = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.value !== "") {
            setTimeout(async () => {
                const fetchUsers = await express.getUsers({
                    input: e.target.value,
                });
                if (!fetchUsers) return;
                const filteredUsers = fetchUsers.filter(
                    (user) => !memberNames.includes(user),
                );
