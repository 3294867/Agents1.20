...
const duplicateThread = async ({
    ...
    try {
        const response = await fetch(
            `${import.meta.env.VITE_EXPRESS_URL}/api/duplicate-thread`,
            {
                method: "POST",
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Failed to duplicate thread: ${response.status} ${response.statusText} - ${errorText}`,
            );
        }

        const data: {
            message: string;
            data: { thread: Thread; agentName: string } | null;
        } = await response.json();
