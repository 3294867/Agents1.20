...
const updateRequestBody = async ({
    ...
    try {
        const response = await fetch(
            `${import.meta.env.VITE_EXPRESS_URL}/api/update-request-body`,
            {
                method: "POST",
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Failed to update request body (PostgresDB): ${response.status} ${response.statusText} - ${errorText}`,
            );
        }

        const data: { message: string; data: null } = await response.json();
        if (data.message !== "Request body updated")
