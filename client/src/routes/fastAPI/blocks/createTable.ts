...
const createTable = async ({
    ...
    try {
        const response = await fetch(
            `${import.meta.env.VITE_FASTAPI_URL}/api/create-table`,
            {
                method: "POST",
        ...
        if (!response.ok)
            throw new Error(`Failed to create table: ${response.text()}`);

        const body: { message: string; data: string } = await response.json();
        if (!body.data)
            throw new Error(`Failed to create table: ${body.message}`);
