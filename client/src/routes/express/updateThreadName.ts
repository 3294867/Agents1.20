...
const updateThreadName = async ({
    ...
    try {
        const response = await fetch(
            `${import.meta.env.VITE_EXPRESS_URL}/api/update-thread-name`,
            {
                method: "POST",
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Failed to update thread name (PostresDB): ${response.status} ${response.statusText} - ${errorText}`,
            );
        }
        const data: { message: string; data: null } = await response.json();
        if (data.message !== "Thread name updated")
