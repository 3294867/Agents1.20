...
const deleteReqRes = async ({
    ...
    try {
        const response = await fetch(
            `${import.meta.env.VITE_EXPRESS_URL}/api/delete-reqres`,
            {
                method: "POST",
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Failed to delete reqres (PostgresDB): ${response.status} ${response.statusText} - ${errorText}`,
            );
        }
