...
const useHandleAgent = ({
    ...
    useEffect(() => {
        ...
        const init = async () => {
            try {
                const workspaceIdIDB = await indexedDB.getWorkspaceId({
                    workspaceName,
                });
                const getAgentIDB = await indexedDB.getAgent({
                    workspaceId: workspaceIdIDB,
                    agentName,
                });
                if (!getAgentIDB) {
                    const getAgentPGDB = await express.getAgent({
                        workspaceId: workspaceIdIDB,
                        agentName,
                    });
                    await indexedDB.addAgent({ agent: getAgentPGDB });

                    const loadSavedTabs = tabsStorage.load({
                        workspaceName,
                        agentName,
                    });
                    if (!loadSavedTabs || loadSavedTabs.length === 0) {
                        const { id, createdAt, updatedAt } =
                            await express.addThread({
                                agentId: getAgentPGDB.id,
                            });
                        ...
                        setIsLoading(false);
                        navigate(`/${workspaceName}/${agentName}/${id}`, {
                            replace: true,
                        });
                    } else {
                        setIsLoading(false);
                        navigate(
                            `/${workspaceName}/${agentName}/${loadSavedTabs[0].id}`,
                            {
                                replace: true,
                            },
                        );
                ...
                if (!loadSavedTabs || loadSavedTabs.length === 0) {
                    ...
                    setIsLoading(false);
                    const activeTabId = loadSavedTabs.filter(
                        (t: Tab) => t.isActive === true,
                    )[0].id;
                    navigate(`/${workspaceName}/${agentName}/${activeTabId}`, {
                        replace: true,
                    });
                }
