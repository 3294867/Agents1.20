...
const useHandleThread = ({
    ...
    useEffect(() => {
        if (
            ...
            setError(
                "All props are required: workspaceId, workspaceName, agentId, agentName, threadId",
            );
            setIsLoading(false);
            return;
        }
        ...
        const init = async () => {
            try {
                ...
                const getThreadPGDBUpdatedAt = await express.getThreadUpdatedAt(
                    {
                        threadId,
                    },
                );
                ...
                if (
                    !getThreadIDB ||
                    new Date(getThreadIDB.updatedAt).getTime() !==
                        new Date(getThreadPGDBUpdatedAt).getTime()
                ) {
    ...
    useEffect(() => {
        const handleReqResAdded = (event: CustomEvent) => {
            if (threadId && event.detail.threadId === threadId) {
                setThread((prevThread) => {
                    ...
                    const prevBody = Array.isArray(prevThread.body)
                        ? prevThread.body
                        : [];
                    return {
                        ...prevThread,
        ...
        window.addEventListener(
            "reqresAdded",
            handleReqResAdded as EventListener,
        );
    ...
    useEffect(() => {
            ...
            if (threadId && event.detail.threadId === threadId) {
                setThread((prevThread) => {
                    ...
                    const prevBody = Array.isArray(prevThread.body)
                        ? prevThread.body
                        : [];
                    const reqresIndex = prevBody.findIndex(
                        (i) => i.requestId === event.detail.reqres.requestId,
                    );
        ...
        window.addEventListener(
            "reqresUpdated",
            handleReqResUpdated as EventListener,
        );
    ...
    useEffect(() => {
            ...
            if (threadId && event.detail.threadId === threadId) {
                setThread((prevThread) => {
                    ...
                    const prevBody = Array.isArray(prevThread.body)
                        ? prevThread.body
                        : [];
                    const updatedBody = prevBody.filter(
                        (q) => q.requestId !== event.detail.requestId,
                    );
                    return {
                        ...prevThread,
                        body: [...updatedBody],
        ...
        window.addEventListener(
            "reqresDeleted",
            handleReqResDeleted as EventListener,
        );
    ...
    useEffect(() => {
        const handleReqResIsNewUpdated = (event: CustomEvent) => {
            if (!thread) return;
            if (threadId && event.detail.threadId === threadId) {
                const threadBody = Array.isArray(thread.body)
                    ? thread.body
                    : [];
                const reqresIndex = threadBody.findIndex(
                    (i) => i.responseId === event.detail.responseId,
                );
                if (reqresIndex === -1) return;

                const isNew: boolean = event.detail.isNew;
                const updatedThreadBody: ReqRes[] = threadBody.map(
                    (item, idx) =>
        ...
        window.addEventListener(
            "reqresIsNewUpdated",
            handleReqResIsNewUpdated as EventListener,
        );
    ...
    useEffect(() => {
        ...
        window.addEventListener(
            "threadNameUpdated",
            handleThreadNameUpdated as EventListener,
        );
    ...
    useEffect(() => {
        ...
        window.addEventListener(
            ...
            handleThreadIsBookmarkedUpdated as EventListener,
        );
