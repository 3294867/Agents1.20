...
const NotificationsList = () => {
    ...
    const { notifications, isLoading, error } =
        hooks.features.useHandleNotifications({ userId });
    ...
    const workspaceInviteNotifications = notifications.filter(
        (i) => i.type === "workspace_invite",
    );

    return (
        <div className={styles.container}>
            <WorkspaceInviteNotifications
                notifications={workspaceInviteNotifications}
            />
        </div>
    );
};
