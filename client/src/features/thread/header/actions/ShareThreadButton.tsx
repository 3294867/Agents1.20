...
const ShareThreadButton = memo(() => {
    ...
    const handleMouseEnter = async () => {
        ...
        try {
            const {
                agentType: publicThreadAgentType,
                threadId: publicThreadId,
            } = await express.addPublicThread({ threadId });
            setSharedThreadId(publicThreadId);
            const agentIDB = await indexedDB.getAgentByType({
                agentType: publicThreadAgentType,
            });
            if (!agentIDB) {
                const agentPostgres = await express.getAgentByType({
                    agentType: publicThreadAgentType,
                });
                if (!agentPostgres) {
                    const getAvailableAgentByType =
                        await express.getAvailableAgentByType({
                            agentType: publicThreadAgentType,
                        });
                    const addAgent = await express.addAgent({
                        workspaceId,
                        agentData: getAvailableAgentByType,
                    });
    ...
    const handleClick = () => {
        setTimeout(() => {
            navigator.clipboard.writeText(
                `${import.meta.env.VITE_CLIENT_URL}/${agentName}/${sharedThreadId}?share=true`,
            );
    ...
    return (
        <Tooltip.Root>
            ...
            <Tooltip.Content>Share chat</Tooltip.Content>
        </Tooltip.Root>
    );
});
