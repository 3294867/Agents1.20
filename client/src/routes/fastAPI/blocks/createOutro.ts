...
const createOutro = async ({
    ...
    try {
        const response = await fetch(
            `${import.meta.env.VITE_FASTAPI_URL}/api/create-outro`,
            {
                method: "POST",
        ...
        if (!response.ok)
            throw new Error(`Failed to create outro: ${response.text()}`);

        const body: { message: string; data: string } = await response.json();
        if (!body.data)
            throw new Error(`Failed to create outro: ${body.message}`);
