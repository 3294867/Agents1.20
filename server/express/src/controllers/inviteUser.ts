...
const inviteUser = async (req: Request, res: Response) => {
    ...
    const validationError = utils.validate.inviteUser({
        userName,
        workspaceId,
    });
    if (validationError)
    ...
    `,
        ...
        if (getWorkspaceName.rows.length === 0)
            utils.sendResponse({
                res,
                status: 404,
                message: "Failed to fetch workspaceName",
            });
    ...
    `,
        ...
        if (addNotification.rows.length === 0)
            utils.sendResponse({
                res,
                status: 503,
    ...
    `,
        ...
        if (addUserNotification.rows.length === 0)
            utils.sendResponse({
                res,
                status: 503,
                message: "Failed to add user_notification",
            });
