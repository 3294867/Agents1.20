...
const getAvailableAgents = async (
    ...
    `,
        ...
        if (getExistingAgentIds.rows.length === 0)
            return utils.sendResponse({
                res,
                status: 404,
                message: "Failed to get existing agent ids",
            });
        const existingAgentIds: string[] = getExistingAgentIds.rows.map(
    ...
    `,
        ...
        if (getExistingAgentTypes.rows.length === 0)
            return utils.sendResponse({
                res,
                status: 404,
                message: "Failed to get existing agent types",
            });

        const existingAgentTypes: AgentType[] = getExistingAgentTypes.rows.map(
            (i: { type: AgentType }) => i.type,
        );
    ...
    `,
        ...
        const rootAgentIds = getRootAgentIds.rows.map(
            (i: { agent_id: string }) => i.agent_id,
        );
        if (getRootAgentIds.rows.length === 0)
            return utils.sendResponse({
                res,
                status: 404,
                message: "Failed to get root agent ids",
            });
    ...
    `,
        ...
        if (getAvailableAgentsData.rows.length === 0)
            return utils.sendResponse({
                res,
                status: 404,
                message: "Failed to get available agents data",
            });
