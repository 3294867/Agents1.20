...
const WorkspaceInviteNotifications = ({ notifications }: Props) => {
    const handleAccept = async (notificationId: string) => {
        const acceptInvite = await express.acceptWorkspaceInvite({
            notificationId,
        });
        if (acceptInvite) {
    ...
    const handleDecline = async (notificationId: string) => {
        const declineInvite = await express.declineWorkspaceInvite({
            notificationId,
        });
        if (declineInvite) {
    ...
    const handleDismiss = async (notificationId: string) => {
        const dismissInvite = await express.dismissWorkspaceInvite({
            notificationId,
        });
        if (dismissInvite) {
            toast.success("Workspace has been added");
