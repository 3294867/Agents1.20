...
const removeThreadName = async ({ threadId }: Props): Promise<void> => {
    try {
        const response = await fetch(
            `${import.meta.env.VITE_EXPRESS_URL}/api/remove-thread-name`,
            {
                method: "POST",
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Failed to remove thread name (PostresDB): ${response.status} ${response.statusText} - ${errorText}`,
            );
        }

        const data: { message: string; data: null } = await response.json();
        if (data.message !== "Thread name removed")
