    ...
    useEffect(() => {
        ...
        const init = async () => {
            try {
                ...
                const getWorkspacesData = await express.getWorkspacesUpdatedAt({
                    userId,
                });

                if (
                    getWorkspacesIDB.length === 0 ||
                    JSON.stringify(workspacesDataIDB) !==
                        JSON.stringify(getWorkspacesData)
                ) {
                    const getWorkspacesPGDB = await express.getWorkspaces({
                        userId,
                    });
                    await indexedDB.addWorkspaces({
                        workspaces: getWorkspacesPGDB,
                    });
