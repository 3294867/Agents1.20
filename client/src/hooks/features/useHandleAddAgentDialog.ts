...
const useHandleAddAgentDialog = ({
    workspaceId,
}: Props): {
    availableAgents: AddAgent[] | null;
    error: string | null;
    isLoading: boolean;
} => {
    const [availableAgents, setAvailableAgents] = useState<AddAgent[] | null>(
        null,
    );
    ...
    useEffect(() => {
        ...
        const getAvailableAgents = async () => {
            try {
                const getAvailableAgents = await express.getAvailableAgents({
                    workspaceId,
                });
